...
- **三源金额单位统一为「元」**：东财 `f52`、新浪 `netamount` 都是元，百度 `extMainIn` 带「万/亿」后缀。带中文数量级的金额解析只有一处实现（`data/parsing.py::parse_money_amount`，crawler 不得再私建副本）；三源同日同值的等价性由 `tests/test_capital_flow_crawler.py` 锁定。
- **变体矩阵与补日都有上限**：单票东财变体**组合**数封顶 `EASTMONEY_VARIANT_MAX_COMBINATIONS`（按组合而非传输计数，否则双传输形态会把预算全花在第一个端点、`push2 kline` 备用端点永远轮不到；不封顶时 403/429 这类非网络层失败会让单票卡满全部组合 × timeout）；百度补日按「最近 N 个缺失日」封顶 `BAIDU_SUPPLEMENT_MAX_MISSING_DATES`，被砍掉的日期必须写 `date_coverage_shortfall`。最近成功缓存按符号数封顶 `RECENT_SUCCESS_CACHE_MAX_SYMBOLS`。
- 新浪资金流端点走 **https**（实测与 http 同载荷）；同文件其余端点本就是 https，不要再退回明文。
...
全市场日线同步的跳过条件必须同时满足 OHLC 完整和 `float_market_cap` 完整。不能因为某只股票 OHLC 已有就跳过它的市值缺口；市值缺口应随 `/sync/full-market` 或指定 `/fetch/daily-bars` 的日线补齐一并修复。后端计算 `filled_missing_rows` 时应复用任务开始时的仓库完整性快照，避免每个写入批次重复扫仓拖慢数据中心。历史遗留的 null 市值行可一次性跑 `scripts/backfill-market-cap.py` 回填（adata 股本历史 × 收盘价；`--dry-run` 只数不写），让这类股票不再被每轮同步反复重抓。
...
补缺日线 provider 顺序必须以公开 HTTP 爬虫为主：`HttpAStockProvider -> ADataProvider -> AkshareProvider`，provider 实例**全程复用**（`HttpAStockProvider.__post_init__` 持有单个 adapter——东财增强端点的失败熔断计数挂在 adapter 上，每票新建会把全市场补齐拖成小时级）。`HttpAStockFetcher` 内部日 K 主源顺序是**腾讯 → 新浪 → 百度**（2026-09 实测：百度对直连 IP 恒 403 且只认完整浏览器 UA；腾讯/新浪直连稳定、价格口径与仓库一致均为不复权）。两条公开链路的铁律：腾讯 volume 单位是**手**必须 ×100、新浪是**股**直接入库；腾讯不支持北交所段，`920xxx` 必须走新浪（`a_share_market_symbol` 已把 `92` 前缀映射为 `bj`，900xxx 沪 B 仍是 `sh`；实测腾讯 `bj920xxx` 恒返回空 `day`，所以 `bj` 段**直接**走新浪，不要再先打一次腾讯）；腾讯 fqkline `count<=320` 且只返回窗口内**末尾**的最后 N 行，长区间必须按窗口**从尾部向回走**分页（正向游标第一轮就会跳到 `end`，起点之前的整段被静默丢掉；`astock_adapter._fetch_tencent_kline`），回走带四道终止护栏：批次最早行必须严格后退（否则 `cursor did not advance`）、空批即停、`TENCENT_KLINE_MAX_PAGES` 封顶、`TENCENT_KLINE_WALK_BUDGET_SECONDS` 墙钟预算封顶（页数上限不是时间上限，异常上游最坏 64 × 15s），护栏触发时日志带 `(under-coverage)` 后缀；新浪 `datalen` 按"最新 N 行"取数，长度按窗口跨度动态算。公开 XHR 日 K 不带市值：用 `qt.gtimg.cn` 报价推导 `float_shares×close` 填充，但**已有逐行市值时必须保留**（百度按每行 `volume/(turnover/100)×close`，反映当日股本；报价法用当前股本乘历史价，解禁/增发后会系统性偏大）。同理公开 XHR 不带换手率：`_with_derived_columns` 必须显式把它置 NaN（否则 importer 的缺列默认 `0.0` 会写成"换手率 0%"假值，污染 `turnover_between` 与候选打分），再由 `_apply_turnover_rate` 用 `volume/流通股×100` 补，量纲是**百分数**（仓库实测 median≈0.38）。`is_st` 上游不带，必须由证券简称派生（`data/symbols.py::is_st_name`，`ST` 子串口径、**不含**"退"字——退市整理期涨跌幅仍是 10%，标成 `is_st` 会给错的 5% 涨跌停）；不派生时 ST 股按 10% 判涨跌停且 `exclude_st` 静默失效。`listing_days` 只认东财 `f189`，拿不到就保持 9999。东财增强（spot 信息 + 120 日资金流）是可选层，连续失败达到 `EASTMONEY_ENRICHMENT_FAILURE_LIMIT` 必须熔断跳过，不要每票白等满超时。`curl_cffi` 备用传输必须经 `http_transport.curl_verify_kwargs()` 注入可加载的 ASCII CA 路径——certifi 装在中文用户名目录下时 libcurl 会报 `curl: (77)` 使整条备用传输失效（守卫在 `tests/test_http_transport.py`）。百度日 K 保留 `curl_cffi` 浏览器 TLS 指纹作为同一 HTTP 主源内的备用，不要因为普通 requests 403 就直接跳到 adata/AKShare；`adata` 数据可能只覆盖到 2025 年底，不能放在近期补缺主路径第一位；AKShare 只能作为最后保底。所有来源都失败或返回空时，错误必须聚合展示每个 provider 的尝试结果，不能只把 AKShare 的断连显示成唯一失败原因。`HttpAStockFetcher` 的注入式构造（`json_get`/`public_json_get`/`public_text_get` 任一传入）必须完全离线：未注入的传输一律停用，绝不允许用例静默打到真实网络。
...
`Warehouse.coverage()` 缺失行口径（1.5.2 起修订）：**可行动缺口与停牌类缺行分列**。日线 = 可行动缺口（停更尾部 + thin day 内部洞）记入 `missing_rows`，停牌类缺行单列 `suspension_rows`；市值/资金流保留“已有行但字段为空”的内部统计 + 停更尾部（资金流尾部边界取 OHLC 末行与资金流末行的较大者——§8 允许“暂无日 K 先写资金流独立行”，独立行覆盖的日期不算缺失）。分类依据是横截面证据：某交易日全市场 OHLC 完整行数 ≥ `median × 0.5` 为市场正常日，其缺行是停牌（公开渠道天然没有停牌 K 线，不可补）；行数异常的交易日是疑似写入失败（thin day），缺行可补。实测依据：2025 年 15,095 个缺口对 **100%** 落在市场正常日——旧“累计真实缺口”口径（停牌日也计为缺失）由此被实证推翻修订，因为那个数字基本虚假且任何渠道都补不上。旧决策中仍然成立的部分：停更尾部必须可见（“多久没同步”的可行动信号），它保持独立统计、绝不参与停牌分类。无生命周期记录的股票保守口径不变（尾部照算、内部洞随当日分类）。补齐入口以缺口为基准：`SyncJobManager.incomplete_symbols(start, end)` 返回窗口内不完整的股票名单，只对名单发起抓取，不做全量扫描。名单内每股的抓取窗口再按「缺失交易日 ± 缓冲」收窄到缺口附近（`sync._narrow_fetch_window`，缓冲天数以 `sync.MISSING_FETCH_BUFFER_DAYS` 为准，两端夹回任务窗口），不再整窗重抓；仓里零行的股票（首次引导）在快照里没有缺口记录，保持整窗抓取，完成判定与 `filled_missing_rows` 仍按任务窗口算、不随收窄变化。这套横截面分类已抽成 warehouse 的共享函数，三个出口必须走同一份：`coverage()`、数据中心逐股明细 `build_daily_bars_coverage` 的 `missing_trade_dates`、以及决定 `incomplete_symbols` 名单的同步完整性快照——市场正常日的缺行不进名单也不进逐股缺口，thin day 缺日与停更尾部照算；窗口交易日过少或分类拿不到时各自退回平日历旧口径。**喂给分类器的计数必须先剔除 0 行日**：日历补出的 0 值不是横截面证据（通常是节假日表覆盖问题或全市场未写入），混进中位数会把阈值压歪、让各出口对同一天给出不同分类；`market_trade_date_counts` 的出口按 `count > 0` 过滤后再喂（warehouse 资金流名单、operations 逐股覆盖），sync 快照本来就只统计窗口内有行的日期。若再出现"覆盖表说没缺、同步却在反复重抓"，先查是不是有出口绕过了共享分类。名单内的股票被 provider **正常返回空帧**（如整窗停牌）记 `skipped` 不记 `failed`，只有抛异常才算失败——否则每轮全市场同步都会以 `completed_with_errors` 收场，看起来像链路坏了。缺口名单算出来**为空**时不得回落全市场：空集的含义是"这个窗口没有洞"，再抓一遍全市场是纯浪费；只有本地根本没有 OHLC 行（首次引导）或缺口检查本身抛异常时才允许回落全量——旧实现把这三种情况混成同一个 `return all_symbols`，防重提。
...
资金流缺口名单 `read_capital_flow_missing_symbols`（`/fetch/capital-flow` 的名单来源）在同一套横截面分类上叠**两条豁免**，且豁免只减不增可见性：①**资金流源起点滞后**——`uses_symbol_capital_flow_source_start` / `uses_listing_day_capital_flow_source_start` 判定该股数据源本身还没到起始日（新股、920 段等）时，源起点之前的日期不算缺口；②**停牌豁免**——市场正常日的整行缺失按停牌类剔除（公开渠道天然没有停牌资金流）。两者都**不豁免**：已有完整日线行但资金流为空的内部洞、thin day 的整行缺失、以及 `max(OHLC 末行, 资金流末行)` 之后的停更尾部；计数拿不到或窗口交易日不足时分类退空集 = 平日历口径，一个豁免都不生效、缺口照算。候选股票仍以「窗口内至少有一行」为界（整窗无行属日线停更，归 coverage/缺口画像管）。
...
5. **回测条件必须双注册**：`conditions.py` 里每个 condition_id 必须同时有行级 `EVALUATORS` 和向量化 `MASK_BUILDERS`；`tests/test_core.py::test_condition_registry_stays_in_sync` 是守卫，新增条件只改 conditions.py 一个文件。两套实现涉及量纲换算时必须**共用同一个 helper**（如 `turnover_between` 的 `_turnover_ratio`）：分别归一必然漂移，而 engine 的 prefilter 走 MASK，漂移的后果是候选被静默丢掉（守卫 `test_turnover_between_row_and_mask_agree_on_percent_scale`）。
...
| 用腾讯回走的"最早一行"当上市日（`listing_days` 的免请求兜底） | 长期停牌横跨预热窗口的老股与真次新股在回走上**无法区分**（已实测误报：把 2026-09-11 当成老股上市日）；写错会经 `derive_listing_dates_from_frame` 污染 `symbol_lifecycle`，比保持 9999（未知）危险得多 | 有独立的上市日字段来源（如 adata `all_code()` 进补缺主路径）时 |
| 给 `turnover_between` 的两套实现各写一份量纲归一 | 行级与向量化分别归一必然漂移（实测：2% 换手率行级 True、MASK False，prefilter 把候选全部丢掉）；归一必须共用 `conditions._turnover_ratio` | 不再需要双注册时（与 §15-5 冲突，须先改该不变量） |
